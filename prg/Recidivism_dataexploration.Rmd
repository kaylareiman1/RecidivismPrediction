...
recid.all <- mutate(recid.all, crime_factor = factor(c_charge_degree)) %>%
      ...
      mutate(decile_score = factor(score_text != "Low", labels = c("LowScore","HighScore")))
...
recid.vio <- mutate(recid.vio, crime_factor = factor(c_charge_degree)) %>%
      ...
      mutate(gender_factor = factor(sex, labels= c("Female","Male"))) %>%
      within(gender_factor <- relevel(gender_factor, ref = 2)) %>%
      mutate(decile_score = factor(v_score_text != "Low", labels = c("LowScore","HighScore")))
```
...
kable(colnames.all, caption = "list of variables in the recid.all dataset")
...
dec.v.factor.all <- ggplot(recid.all, aes(x=decile_score, fill = score_factor))
dec.v.factor.vio <- ggplot(recid.vio, aes(x=decile_score, fill = score_factor))
...
# Univariate #
##############
sex.uni.all <- ggplot(recid.all, aes(x=sex, fill = sex))
sex.uni.all + 
  geom_bar(stat = "count", na.rm = F) + 
  ggtitle("Univariate Sex Breakdown") + 
  guides(fill = FALSE) + 
  xlab("Sex") + 
...
sex.bi.all.dec <- ggplot(data = sex.bi.conf.all, aes(x = sex, y = Avgdecile_score,fill = sex)) +

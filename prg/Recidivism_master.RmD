...
```{r}
kable(colnames(recid.new), col.names = "All variables")
```
...
***Model outcome***  
Based on the findings above, we trained a logistic regression model on the training dataset. Results are below. The histogram shows the distribution of estimated probabilities of recidivism from our logistic regression. 
...
We then evaluated our classifier by examining how often the logistic model misclassified people. We also looked at both specificity and sensitivity, which can be calculated via the confusion matrix and then represented via an ROC curve. An overview of these measures is below:  
   ...
   -**Sensitivity**: This is the percentage of high-risk individuals who are correctly classified by the model. A highly sensitive model would correctly predict most individuals who will recidivate within two years. We consider sensitivity a more important metric for our violent recidivism model than for the full recidivism model.  
...
The following ROC chart shows that our logistic regression model will successfully classify a randomly selected recidivator as a higher risk than a randomly selected non-recidivator 74% of the time. This is a reasonable classifier. However, as we change the threshold, the relative values of Specificity and Sensitivity change accordingly. Please refer to this ROC chart as you progress through the logistic regression section.
<span style ="color: red;">**I modified the formatting here to improve readability -- if you like it, let's copy/paste the formatting to the other ROCs.**</span>
...
The following are the results when setting an **``r scales::percent(logit.threshold)``** threshold:
...
Again, by increasing the threshold to ``r scales::percent(logit.threshold)``, meaning that the model will mark individuals as "1"--will recidivate within two years-- if they have a probability of recidivating of ``r scales::percent(logit.threshold)`` or higher, our Sensitivity decreases to `r scales::percent(logit.sens, accuracy = 0.1)`, and complementing it, our Specificity increases to `r scales::percent(logit.spec, accuracy = 0.1)`. 
Accuracy measures how often the prediction matches the outcome (did or did not recidivate, 1 or 0). The most accurate model uses a 50% threshold because the model will be matching as often as possible. However, sensitivity and specificity are also important factors in a model.
...
The ROC chart shows that the AUC score is ``r round(auc(roc_lda),1)``%, a value greater than 0.5 meaning that the LDA model is reasonable measure of separability. Furthermore, it shows us that the optimal threshold for the LDA model is at 51.9% instead of 50%, which would achieve a higher Specificity score. 
...
roc_qda <- roc(response = val.dat$any_recid_2yr, predictor = qda.cv.pred$posterior[,"Yes"])
plot(roc_qda,
     col="darkblue",
     lwd = 3,
     asp = 0,
...
#auc(roc_qda)
```
The ROC chart for the QDA model suggests that the optimal cutoff value is at 21.8%. Since this would suggest giving a higher rating to an individual who has a 21.8% chance of recidivating within two years, we are less likely to deem this model appropriate for this dataset.
...
However, this is only the first step in the process. We must next re-run the model using our validation data with (with ``r nrow(val.dat)`` rows) to find a confusion matrix.
...
The regression tree is now shown below using the `as.party` command to increase interpretability.
```{r}
#Output pretty picture - must first run using rpart -- a recursive partitioning function that outputs an object that have pretty formatting.
rpart.recid.reg <- rpart(formula = num.recidany ~
                         crime_factor +
                         age +
                         age_factor +
                         race_factor +
                         ...
                         length_of_stay,
                       data = train.dat)
reg.tree.recid.pretty <- as.party(rpart.recid.reg)
plot(reg.tree.recid.pretty, pretty = 1)
```

**Validation: regression tree**  
```{r}
#Predict and use the validation data
reg.tree.recid.val   <- predict(reg.tree.recid, val.dat)
#Create ROC curve
roccurve.trpart<- roc(response = val.dat$two_year_recid,
                      predictor = reg.tree.recid.val[,2],
                      levels = c(0, 1),
                      direction = "<")
...
From the ROC chart we do see that the AUC value is lower for the regression tree than for the logistic regression or LDA models. The regression tree allows for more nuance than the classification tree. In addition to having more branches, it also has numeric rankings of likelihood of recidivism, rather than just saying that all people under the age of 23 will recidivate. 
<span style ="color: red;">**KR note: tried to add the requested nuance here. Somebody please proofread**</span> 
...
For the purposes of this project, we focused on classification tress. The ISLR book and Data Mining lectures focused on random forest models for regression trees, which while helpful for understanding concepts, was not sufficient for the purposes of this project. 
...
roccurve.rpart<- roc(response = val.dat$two_year_recid, predictor = rf.predict$Yes)
...
<span style ="color: red;"> Please remove this chart if you do not feel that it provides any extra value.</span> 
```{r}
...
#par(pty="s")
plot(roccurve.logit, grid = TRUE, col = "blue", lty = 11, main = "Comparing ROC Curves", print.thres = TRUE)
plot(roc_lda, col = "red", lty = 3, print.thres = TRUE, add = TRUE)
plot (roc_qda, col = "green", lty = 10, add = TRUE)
plot (roccurve.trpart, col = "purple", lty = 3, add = TRUE)
plot (roccurve.rpart, col = "brown", lty = 2, add = TRUE)
legend(0.3, 0.42, c("Logistic Regression", "LDA", "QDA", "Regression Tree", "Random Forest"), lty = c(1,1), 
       lwd = c(2,2), col = c("blue", "red", "green", "purple", "brown"))
...
```{r, message = FALSE}
kable(recid.new%>%
  count(any_recid_2yr, two_year_recid, num.recidany), caption = "Outcome variable 1: any recidivism within 2 years")
kable(recid.new%>%
  count(vio_recid_2yr, two_year_recid, is_violent_recid, num.recidvio), caption = "Outcome variable 2: violent recidivism within 2 years")
kable(recid.new%>%
...
<span style ="color: red;"> I definitely think this should all be mentioned and we could write it in a little paragraph below this ROC curve under a little heading like "Phylosphical Analysis," what do you think? - SY.</span> 
...
#Run REGRESSION tree
reg.tree.recidvio <- tree(formula = num.recidvio ~
                         crime_factor +
                         age +
                         age_factor +
                         race_factor +
...
#Create ROC curve
roccurve.rpart<- roc(response = train.dat$num.recidvio, predictor = tree.pred.rpart.train)
...
plot(reg.tree.recidvio)
text(reg.tree.recidvio, pretty = 0)
#Predict and use the val data
tree.pred.rpart.val   <- predict(reg.tree.recidvio, val.dat)
#Create ROC curve
roccurve.rpart<- roc(response = val.dat$num.recidvio, predictor = tree.pred.rpart.val)
#Plot ROC 
plot.roc(roccurve.rpart, print.auc = TRUE, print.thres = TRUE)
```

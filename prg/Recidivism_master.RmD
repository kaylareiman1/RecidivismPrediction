...
| <span style ="color: red;"> Logistic regression |<span style ="color: red;">`r scales::percent(logit.acc, 0.1)`</span> |<span style ="color: red;"> `r scales::percent(logit.sens, 0.1)`</span>   |<span style ="color: red;">`r scales::percent(logit.spec)`</span> |  
...
|<span style ="color: red;"> Logistic regression</span>|<span style ="color: red;">`r scales::percent(logit.acc.vio, 0.1)`</span> |<span style ="color: red;">`r scales::percent(logit.sens.vio, 0.1)`</span> |<span style ="color: red;">`r scales::percent(logit.spec.vio, 0.1)`</span>|  
...
## **2d: Demographic analysis of the best version of each model:**  
...
We will reproduce this table using our own data and hope to have less inequality than was shown in the replicated ProPublica chart below. 

| ProPublica's COMPAS finding                |           White | African American | 
|--------------------------------------------|-----------------|------------------|
...
<span style ="color: red;">We don't happen to have ProPublica Violent Recidivism stats for race do we?</span>
...
<span style ="color: red;"> Ah! 45 and up has 100% False Negative rate!!?? Help here! not sure what went wrong</span>
...
Below, the accuracy, sensitivity, and specificity rates are shown of the COMPAS model.  
```{r, warning = FALSE, message = FALSE}
#Create table
confusions.compas.any = table(predicted = recid.new$score_factor, actual = recid.new$any_recid_2yr)
dimnames(confusions.compas.any)[[1]] = c("No (Predicted)","Yes (Predicted)")

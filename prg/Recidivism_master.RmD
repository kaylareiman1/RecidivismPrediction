...
age_factor.bi.conf.all <- recid.new %>%
  ...
  summarize(Avgdecile_score = mean(decile_score),
            upper = t.test(decile_score)$conf.int[1],
            lower = t.test(decile_score)$conf.int[2])
age_factor.bi.all.dec <- ggplot(data = age_factor.bi.conf.all, aes(x = age_factor, y = Avgdecile_score, fill = I("darkorange"))) +  geom_bar(stat = "identity") +
  xlab("Age") + 
  ylab("Average prediction") +
  ggtitle("Prediction from COMPAS") + 
  guides(fill = FALSE) +
...
biplot(recid.pca,
       scale = 0,
       cex = c(0.8, 1),
       col=c("darkorange", "dodgerblue4")) # construct biplot
## Add grid lines
...
The logistic regression produces probabilities that an individual will recidivate in the two year time span. This allows us to tune a threshold and observe different model accuracies with different thresholds.
...
The following ROC chart shows that the logistic regression set at a threshold of 46.2% will classify a randomly selected observation where an indvidual did recidivate higher than a randomly selected individual who did not recidivate within two years with a probability of 74%. This is a reasonable classifier. However, as we'll see, perhaps the accuracy of the model is not what is most important for this model, and as we change the threshold, we observe that Specificity and Sensitivity change accordingly. Please refer to this ROC chart as you progress through the logistic regression section.
<span style ="color: red;">**If anyone could proof read that little paragraph, that would be greatly appreciated!**</span>  

```{r, message = FALSE}
glm.logit.predict.val  = predict(glm.logit, val.dat, type="response")
## ROC plot
roccurve.logit <- roc(response = val.dat$any_recid_2yr, predictor = glm.logit.predict.val)
#plot.roc(roccurve.logit, color = "red", lwd = 3, main= "Logistic Regression - ROC Chart", print.auc = TRUE, print.thres = TRUE)
plot(roccurve.logit, color = "blue", lwd = 3, main= "Logistic Regression - ROC Chart", print.auc = TRUE, print.thres = TRUE)
...
roc_lda <- roc(response = val.dat$any_recid_2yr, predictor = lda.pred.val$posterior[,"Yes"])
plot(roc_lda, col="red", lwd = 3, main="LDA - ROC Chart", print.auc = TRUE, print.thres = TRUE)
auc(roc_lda)

```

The ROC chart shows that the AUC score is `r auc(roc_lda)`, a value greater than 0.5 meaning that the LDA model is reasonable measure of separability. Futhermore, it shows us that the optimal threshold for the LDA model is at 51.9% instead of 50%, which would achieve a higher Specificity score. 
...
plot(roc_qda, col="red", lwd = 3, main="QDA - ROC Chart", print.auc = TRUE, print.thres = TRUE)
#auc(roc_qda)
...
**<span style ="color: red;">We probably don't need this regression tree, but here it is in case we want it! Let's decide if we want this or not! We need to explain the outcome of this tree if we keep it</span>**
```{r}
#Run REGRESSION tree
...
print(tree.recid.rpart.train.pretty)
#Predict and use the validation data
tree.pred.rpart.val   <- predict(tree.recid.rpart.train, val.dat)
#Create ROC curve
roccurve.trpart<- roc(response = val.dat$two_year_recid, predictor = tree.pred.rpart.val[,2], levels = c(0, 1), direction = "<")
#plot.roc(roccurve.trpart, print.auc = TRUE, print.thres = TRUE)
plot(roccurve.trpart, col="green", lwd = 3, main="Tree - ROC Chart", print.auc = TRUE, print.thres = TRUE)
...
From the ROC chart we do see that the specificity, sensitivity, and AUC values are not as high as they are for the logistic regression or LDA models. 
<span style ="color: red;">**Could someone write a quick sentence about what information the tree is providing us here?**</span> 

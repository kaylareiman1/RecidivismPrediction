...
We then evaluated our classifier by looking at how often the logistic model misclassified people. However, we also looked at both specificity and sensitivity, which can also be calculated via the confusion matrix and then represented via an ROC curve. An overview of why all of these measures are relevant is below:  
   -**Accuracy**: This is the percent of predictions that were correct. It is a limited measure of the tool's usefulness because it counts false positives and false negatives in the same way, when their human impact is much different. That said, it is the simplest measure. The accuracy rate is 1 - the misclassification rate.    
    -**Specifity**: This is the true negative rate. A higher value shows that we are not mistakenly classifying people as high-risk. We believe this is extremely important because it keeps innocent people from receiving harsh punishments. For the purposes of recidivism modeling, identifying a model with high specificity is desirable because False Positives (assigning a low-risk individual with a high-recidivism rate) are more costly than False Negatives (failing to high-risk individuals with the right ranking).
   -**Sensitivity**: This shows the true positive rate. A higher value shows that we successfully predicted people would commit crimes. For violent crime, a high sensitivity rate is important.  
...
The following model is tested with a 50% threshold, meaning that the model will identify an individual as will recidivate in two years (1) if they have a 50% or higher percentage likelihood of recidivating:

...
***Another Method in Deciding which Variables to Use:***
**PCA**    
...
## ROC plot
roccurve <- roc(glm.logit$y, train.pred.logit)
plot(roccurve)

```
In the ROC plot we want the probability of True Positives found (Sensitivity) to be as high as possible, while the probability of False Positives (Specificity) to be as low as possible. Therefore, we want to set a threshold that results in the highest TPR and lowest FPR. **NOT SURE IF THIS SHOULD BE INCLUDED IF WE DON'T DO WORK SETTING A THRESHOLD...AND I DON'T KNOW HOW TO DO THAT.**



**Model Validation**  
**Update needed: edit this text to explain the CV process. Also, should we stop the output from printing or kable it?**    
We use the model above to make a prediction for any_recid_2yr (using the cross-validation data)

The predict function is used on our glm logistic regression model to predict the probability that a person will recidivate two years after their initial arrest. The `type="response"` option tells R to output probabilities of the form P(Y=1 | X). The data given to validate the training model is `is.cv = 2` or `cv.dat`, which is a subset of 20% the original dataset. 
...
dimnames(confusions.pred)[[1]] = c("No","Yes")
confusions.pred
...
#logit.tab = table(predicted = glm.logit.predict$class,
...
## ROC curve: "Receiver Operating Characteristics Curve traces the percentage of true positives accurately predicted by a given logit model as the prediction probability cutoff is lowered from 1 to 0 [...] The greater the area under the ROC curve, better the predictive ability of the model."

#plotROC(cv.dat$any_recid_2yr, glm.logit.predict)
paste("Accuracy:")
logit.acc <- sum(diag(confusions.pred)) / sum(confusions.pred)
logit.acc
paste("Sensitivity Rate:")
logit.sens <- sensitivity(confusions.pred)
logit.sens
paste("Specificity Rate:")
logit.spec <- specificity(confusions.pred)
logit.spec
...
The model run with the validation dataset gives us a better accuracy than the training dataset. 
...
recid.rf <- randomForest(any_recid_2yr ~ age + juv_fel_count + juv_misd_count + juv_other_count + priors_count + crime_factor + age_factor + race_factor + gender_factor, data=train.dat, mtry=9, importance = TRUE)

#plot(margin(recid.rf))
...
#partialPlot()
paste("Importance of each variable:")
imp.recid <- importance(recid.rf)
imp.recid


## From the book
recid.test=cv.dat[,"any_recid_2yr"]
yhat.rf <- predict(recid.rf, newdata=cv.dat)

small.rf <- randomForest(any_recid_2yr ~ age + juv_misd_count + priors_log, data=train.dat, mtry=3, importance = TRUE)
importance(small.rf)

## Plot prediction interaction attempt
#library(randomForestExplainer)
#cv.dat <- na.omit(cv.dat)
#plot_predict_interaction(recid.rf, cv.dat, "rm", "lstat")

## From cran.r-project.org/web/packages/randomForestExplainer/vignettes/randomForestExplainer.html
## I'm going to try with a smaller forest:
## This still did not run for me....it took forever and I quit it
## If interested in figuring out, please do, by all means.
#small.rf <- randomForest(any_recid_2yr ~ age + juv_misd_count + priors_log, data=train.dat, mtry=3, importance = TRUE)

#importance_frame <- measure_importance(small.rf)
...
#                ylim=c(30, 70))
```
It seems like the decrease accuracy
Please see stats.stackexchange.com/questions/197827/how-to-interpret-mean-decrease-in-accuracy-and-mean-decrease-gini-in-random-fore
for better information on how to interpret these.... I will work on this post-exam, definitely, if no one else has figured it out :) 

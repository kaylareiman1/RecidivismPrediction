...
-Do we split data into 3: train, CV for k-fold, and test for comparing accross models?    
  + train for first models   
  + CV for choosing best accuracy   
  + test for comparing models when we care about FPR vs. FNR  
  + Related: How does the video about the importance of splitting the data before starting relate to K-fold CV, which we learned about next as the better alternative?  
  + Related as well: do we use the same break for all types of models? Or redo for each model?  
...
-Can we do the same stepwise forward/backward stuff to choose variables to include in logistic model?  
- GLM: what are all the components that make it general?  
- Is it worth spending time on the raw data instead of trusting ProPublica's cleaning?
...
An overview of variables that ProPublica used is below: [KR note: this dictionary could be better.] 
...
Here we create a column `is.cv` where `train.dat` is a subset of the data whose is.cv value is 0 (70% of the recid.all dataset); `cv.dat` is a subset of the data whose is.cv value is 2 (20% of the recid.all dataset); and `test.dat` is a subset of the data whose is.cv value is 1 (10% of the recid.all dataset):
```{r, message = FALSE}
...
```
  ...
  Based on this finding, we chose to add `log2(priors_count + 1)` as a variable. We also added variables indicating whether the defendant had any cases on their juvenile record. We then performed an exhaustive search to select variables to keep for our model.
```{r}
#Choosing variables
...
points(maxr2, summary(recid.subsets)$adjr2[maxr2], col = "green3", cex = 1, pch = 20)
#BIC Chart
...
Depending on the metric used, the best-performing model uses somewhere between ``r minbic`` (according to BIC) and ``r maxr2`` (according to adjusted r-squared) variables. As the increase in adjusted r-squared is nearly imperceptible between 8 variables and 12 variables, it makes sense to choose 6-8 variables. After examining the models in this range, we went with the following 7 variables (presented with coefficients):
```{r}
kable(coef(recid.subsets, 7), format = "markdown")
```

**Sormeh attempts Forward Stepwise, instead of exhaustive search**

- **For the forward stepwise selection model, we see that the best one-variable model contains:** `log2(priors_count + 1`.
- **We see that the best two-variable model contains:** `log2(priors_count + 1`, `age`.
- **The best three-variable model contains:** `log2(priors_count + 1`, `age`, `age_factorLess than 25`.
- **The best four-variable model contains:** `log2(priors_count + 1`, `age`, `age_factorLess than 25`, `gender_factorFemale`.
- **The best five-variable model contains:** `log2(priors_count + 1`, `age`, `age_factorLess than 25`, `gender_factorFemale`, `length_of_stay`.
- **The best six-variable model contains:** `log2(priors_count + 1`, `age`, `age_factorLess than 25`, `gender_factorFemale`, `length_of_stay`, `juv_other_count`.
- **The best seven-variable model contains:** `log2(priors_count + 1`, `age`, `age_factorLess than 25`, `gender_factorFemale`, `length_of_stay`, `juv_other_count`, `crime_factorM`.
- **The best eight-variable model contains:** `log2(priors_count + 1`, `age`, `age_factorLess than 25`, `gender_factorFemale`, `length_of_stay`, `juv_other_count`, `crime_factorM`, `age_factorGreater than 45`.
- **The best nine-variable model contains:** `log2(priors_count + 1`, `age`, `age_factorLess than 25`, `gender_factorFemale`, `length_of_stay`, `juv_other_count`, `crime_factorM`, `age_factorGreater than 45`, `race_factorAfrican-American`.
- **The best ten-variable model contains:** `log2(priors_count + 1`, `age`, `age_factorLess than 25`, `gender_factorFemale`, `length_of_stay`, `juv_other_count`, `crime_factorM`, `age_factorGreater than 45`, `race_factorAfrican-American`, `priors_count`.
- **The best eleven-variable model contains:** `log2(priors_count + 1`, `age`, `age_factorLess than 25`, `gender_factorFemale`, `length_of_stay`, `juv_other_count`, `crime_factorM`, `age_factorGreater than 45`, `race_factorAfrican-American`, `priors_count`, `race_factorAsian`.
- **The best twelve-variable model contains:** `log2(priors_count + 1`, `age`, `age_factorLess than 25`, `gender_factorFemale`, `length_of_stay`, `juv_other_count`, `crime_factorM`, `age_factorGreater than 45`, `race_factorAfrican-American`, `priors_count`, `race_factorAsian`, `race_factorHispanic`.
- There are differences between the forward stepwise selection model and the exhaustive model. (IDK IF THIS IS TRUE)

```{r, message=FALSE}
recid.forward <- regsubsets(two_year_recid ~ crime_factor + age + age_factor + race_factor + gender_factor + log2(priors_count + 1) + priors_count + juv_fel_count + juv_misd_count + juv_other_count + length_of_stay, data = recid.all, nbest = 1, nvmax = 15, method = "forward")

coef(recid.forward, 5)
summary(recid.forward)

```

```{r, message=FALSE}
rec.fwd.summary <- summary(recid.forward)

plot(rec.fwd.summary$adjr2, xlab = "Number of variables/ Model size", ylab = "Adjusted R^2", type = "l")
points(which.max(rec.fwd.summary$adjr2), rec.fwd.summary$adjr2[which.max(rec.fwd.summary$adjr2)], col = "blue", cex = 2, pch = 20)
```

```{r}
plot(rec.fwd.summary$cp, xlab = "Model Size", ylab = "AIC (or C_p)", type = "l")
points(which.min(rec.fwd.summary$cp), rec.fwd.summary$adjr2[which.min(rec.fwd.summary$cp)], col = "green", cex = 2, pch = 20)
coef(recid.forward, 10)
```
- **Model size** `r which.min(rec.fwd.summary$cp)` **minimizes AIC**
- **The curve is not monotonic. It decreases until model 10, and then starts to increase again after model 10.**
- **There are X noise predictors in this model.** HOW MANY NOISE PREDICTORS??
- **AIC is said to be an unbiased estimate of test MSE, so it generally chooses the smaller number of variables for models with low test error.**
- SHOULD WE ALSO DO BIC???
...
```{r, message = FALSE}
glm.logit <- glm(two_year_recid ~ crime_factor + age + age_factor + race_factor + gender_factor + log2(priors_count + 1) + priors_count + juv_fel_count + juv_misd_count + juv_other_count + length_of_stay, data=train.dat, family = binomial)
summary(glm.logit)

...
Given our task of predicting binary outcomes, we considered the following classification methods:  
   ...
   - Random Forrest (RF): 
   - Classification Tree: Although not as trustworthy as random forrest, the potential for public transparency makes this worth trying.  
   - K-Nearest Neighbors (KNN):    
**Sormeh attempts Forward Stepwise, instead of exhaustive search**

```{r, message=FALSE}
#recid.forward <- regsubsets(two_year_recid ~ compas_screening_date + sex + dob + age + age_cat + race + juv_fel_count + decile_score + juv_misd_count +
#                              juv_other_count + priors_count + days_b_screening_arrest + c_jail_in + c_jail_out + c_offense_date + c_arrest_date + c_days_from_compas +
#                              c_charge_degree + c_charge_desc + is_recid + r_charge_degree + r_days_from_arrest + r_offense_date + r_charge_desc + r_jail_in +
#                              r_jail_out + violent_recid + is_violent_recid + vr_charge_degree + type_of_assessment + screening_date + priors_count.1 + start +
#                              end + event + length_of_stay + crime_factor + age_factor + race_factor + gender_factor + score_factor, 
#                            data = recid.all,
#                            nbest = 1, #1 best model for each number of predictors
#                            nvmax = NULL, 
#                            method = "forward", really.big = TRUE)
## The above will not run, Error in `contrasts` can only be applied to factors with 2 or more levels
## Attempting below by removing variables: 
recid.forward <- regsubsets(two_year_recid ~ sex + age + age_cat + race + juv_fel_count + juv_misd_count +
                              juv_other_count + priors_count + 
                              c_charge_degree + c_charge_desc + is_recid + r_charge_degree + r_days_from_arrest + r_charge_desc +
                              is_violent_recid + priors_count.1 +
                              length_of_stay + crime_factor + age_factor + race_factor + gender_factor, 
                            data = recid.all,
                            nbest = 1, #1 best model for each number of predictors
                            nvmax = NULL, 
                            method = "forward", really.big = TRUE)
...
```
**Sormeh Attempts Naive Bayes**
Could not find how we did this in lab, so following tutorial at uc-r.github.io/naive_bayes
```{r, message = FALSE}
...
```
Bayesian probability incorporates the concept of conditional probability, the probability of event A given that event B has occurred [denoted as P(A|B)]. In the context of our data, we are seeking the probability of an individual belonging to two_year_recid = 1, given that its predictor values are [..... something]
Here's a plot that attempts to show correlation among variables. It is different from the way we've typically done it in class
```{r, message = FALSE}

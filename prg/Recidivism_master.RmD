...
### THIS FANCY PLOT CODING THING I TRIED RIGHT HERE IS FROM 
## benjaminbell.co.uk/2018/02/principal-components-analysis-pca-in-r.html
## I tried, but it isn't working out exactly the way I wanted it to.
## Might just delete this plot altogether, but left it so you could observe the labor of my work, because sadly it took me a decent amount of time
## Create groups
...
## Plot individuals
summary.recid.pca <- summary(recid.pca)
p <- prcomp(recid.new.scaled)
#p$x[,1:2]
s <- summary(p)
#recid.pca
...
**left here because unsure if this is preferred to the one above, the one without the colorful points**
...
Given our task of predicting binary outcomes, this is fundamentally a classification methods. Informed by [our book - ISLR](https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=1&cad=rja&uact=8&ved=2ahUKEwjx8tmdyafpAhVxg3IEHWqjDIsQFjAAegQIAhAB&url=https%3A%2F%2Ffaculty.marshall.usc.edu%2Fgareth-james%2FISL%2FISLR%2520Seventh%2520Printing.pdf&usg=AOvVaw3IIbJOiIiKLgG0eFhvQBp9), we decided to pursue the following methods:  
   ...
   3) **Quadratic Discriminant Analysis (QDA)**: This is like LDA but more flexible. It works when there are class-based interactions among the predictors, so we will try this out in addition to LDA in case the model is superior.   
   4)  **Classification Tree**: Using a decision tree to classify our respondents is helpful if the relationship between variables is non-linear. Some may also argue this better approximates human decision making. An additional benefit is that they can be interpreted graphically, which is important when creating a model for something that requires public transparency. However, thie disadvantage is that trees are unstable. That is, a few new observations can completely change which variable is being used for the first split.
   5) **Random Forest (RF)**: This model improves the stability of the classification tree by essentially creating a bunch of different trees with random variables available to them. If our data are best modeled by a tree, the random forrest will likely give a lower error rate than a single classificaiton tree.
...
***Model outcome***  
Based on the findings above about which variables to include, we ran the logistic regression on our training dataset. Results are below. The histogram shows us the distribution of estimated probabilities of recidivism from our logistic regression. 
```{r, echo = FALSE}
...
qplot(train.pred.logit, geom="histogram")
```

We then evaluated our classifier by looking at how often the logistic model misclassified people. However, we also looked at both specificity and sensitivity, which can also be calculated via the confusion matrix and then represented via an ROC curve. An overview of why all of these measures are relevant is below:  
   -**Accuracy**: This is the percent of predictions that were correct. It is a limited measure of the tool's usefulness because it counts false positives and false negatives in the same way, when their human impact is much different. That said, it is the simplest measure. The accuracy rate is 1 - the misclassification rate.    
    -**Specifity**: This shows the true negative rate. A higher number shows that we are not mistakenly classifying people as high-risk.  general, we believe this is extremely important because it keeps innocent people from receiving harsh punishments.    
...
#Commenting out for now since don't have a good explanation for alpha -- but let's bring it back if we have time!
## ROC plot
# logit.val.roccurve <- roc(glm.logit$y, val.pred.logit)
# plot(logit.train.roccurve)
# plotROC(val.dat$any_recid_2yr, glm.logit.predict.val)

#ROC explanation:
# In the ROC plot we want the probability of True Positives found (Sensitivity) to be as high as possible, while the probability of False Positives (Specificity) to be as low as possible. A large distance from the 45 degree line shows that our predictor is doing better than a random coin flip.  
```
...
qda.cv.pred <- predict(object = recid.qda,
                       newdata = val.dat,
                       type = "response")
qda.cv.tab.val = table(predicted = qda.cv.pred.val$class,
...
Building on the classification tree work above, we now assess the data using random forrests. Although these models are substantially less user-friendly than single trees, they are helpful for reducing variance and are much more flexible than individual trees. The idea behind random forests is the software randomly selects different predictors for each split, and this process happens multiple times. The trees are decorrelated from each other because the software tries different splitting variables at different times.

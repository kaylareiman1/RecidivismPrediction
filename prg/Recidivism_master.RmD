...
* Assess whether the RAIs for any recidivism and specifically violent recidivism are equally predictive across race/ethnicity, age, and gender_factor groups.
...
logit.threshold <- 0.6
```
In line with our goal of increasing specificity, we now increase the threshold to ``r scales::percent(logit.threshold)``.  
...
Unlike modeling whether a picture is of a cat or a dog, which has ground truth, predicting whether an individual will recidivate is inherently random. For this reason, accuracy is not the best measure of how good our model is, and logistic regression (italicized in the chart below) appears to be the best model for this type of data.  
...
From the Gender table below we see that our model strongly favors giving female individuals a lower recidivism risk rating. The False Negative rate of ``r scales::percent(FN.any.female, 0.1)`` is especially telling, indicating that our model correctly identifies only ``r round(100*(1-FN.any.female))`` of every hundred recidivating women. Our model's sensitivity rate for males is ``r round((1 - FN.any.male) / (1 - FN.any.female), 1)`` times higher than for females, meaning it is ``r round((1 - FN.any.male) / (1 - FN.any.female), 1)`` times more likely to successfully detect risk in men than in women.  

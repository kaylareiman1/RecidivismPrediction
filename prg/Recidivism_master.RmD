...
#rf.vals$sensitivity
```
The Random Forest model does seem to be a reasonable classifier, on par with the logistic regression and LDA models.

```{r, warnings = FALSE}
rf.predict <- as.data.frame(predict(recid.rf, test.dat, type = "prob"))
#Adapting code from other parts of the program
roccurve.rpart<- roc(response = test.dat$two_year_recid,
                     predictor = rf.predict$Yes,
                     percent = TRUE)
plot.roc(roccurve.rpart,
         print.auc = TRUE,

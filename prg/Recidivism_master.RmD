...
**<span style = "color: red;">I commented out Misclassification Rate because it seems redundant if we also include accuracy in our nice little tibble. Also, I think this section would look less cluttered if we only included a confusion matrix for the first one -- do you think it's enough to just include the acc/sens/spec tibble for the other two? -MH</span>**  
...
**<span style = "color: red;">These were mixing up sensitivity and specificity due to formatting errors, so I fixed these. I also made `logit.threshold` a variable so it's easy to modify the threshold in our code (e.g. for the violent recid). -MH</span>**  
...
Just like with the full recidivism dataset, we also ran a regression tree. This single tree makes it clear how the two relate.
```{r}
#Run REGRESSION tree
...
```{r}
confusions.tree.vio <- table(predicted = tree.pred.prune, actual =val.dat$vio_recid_2yr)
dimnames(confusions.tree.vio)[[1]] = c("No (Predicted)","Yes (Predicted)")
confusions.tree.vio
tree.pred.prune.errors = confusions.tree.vio[2,1] + confusions.tree.vio[1,2]
```
This final confusion matrix produced after pruning shows ``r tree.pred.prune.errors`` errors out of ``r nrow(val.dat)`` observations, for a test error rate of ``r scales::percent(tree.pred.prune.errors/nrow(val.dat), accuracy = 0.1)``.
```{r}
#paste("Accuracy:")
tree.acc <- sum(diag(confusions.tree.vio)) / sum(confusions.tree.vio)
#logit.acc
#paste("Sensitivity Rate:")
tree.sens <- confusions.tree.vio["Yes (Predicted)", "Yes"] / sum(confusions.tree.vio[,"Yes"])
#paste("Specificity Rate:")
tree.spec <- confusions.tree.vio["No (Predicted)", "No"] / sum(confusions.tree.vio[,"No"])
#logit.spec
kable(tibble(Accuracy = scales::percent(tree.acc, accuracy = 0.1),
             Sensitivity = scales::percent(tree.sens, accuracy = 0.1),
             Specificity = scales::percent(tree.spec, accuracy = 0.1)),

...
```
  ...
  Based on this finding, we chose to add `log2(priors_count + 1)` as a variable. We also added variables indicating whether the defendant had any cases on their juvenile record. We then performed an exhaustive search to select variables to keep for our general linear model. In this case, since our outcome variable `two_year_recid` is binary, we will be aplying a logistic regression to classify participants.
```{r}
#FOR DISCUSSION: This is for a linear regression, right? I think we have to choose variables separately for each model, rather than just choosing them once. 
#Choosing variables
#FOR DISCUSSION: I think we're not SUPPOSED to use both age and age_factor b/c the whole thing we learned in HW 1 about collinearity. 
...
#FOR DISCUSSION: Per fomments above, could this be where we choose variables?
...
#FOR DISCUSSION: Can we do the partimat thing here? It doesn't seem to work, but this may be an error in my code. It was so pretty on the hw!
#example
#partimat(Species ~ ., data = iris, method = "lda")
# partimat(two_year_recid ~
#                    crime_factor +
#                    age +
#                    (age_factor == "Less than 25") +
#                    gender_factor +
#                    length_of_stay +
#                    (juv_other_count > 0) +
#                    log2(priors_count + 1), data = recid.all, method = "lda")

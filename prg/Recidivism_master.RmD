...
* Assess whether the RAIs for any recidivism and specif are equally predictive across race/ethnicity groups? How about across age and gender_factor groups?
...
The confusion matrix below shows the prediction abilities of the COMPAS, when people were divided into the categories of "low" and "medium/high" for total recidivism. Additional analysis of the COMPAS's predictions will be included in the comparison of our model against theirs. However, this initial analysis shows 2345 true negatives (correctly identifying people as low risk), 1733 true positives (correctly identifying people as high risk), and just over 1,000 false negatives as well as false positives. 
...
Despite the pairs resulting in low correlation, we still attempt PCA to see the impact of the correlations present:
...
The ROC chart shows that the AUC score is ``r round(auc(roc_lda),1)``%, a value well above 50% meaning that the LDA model is a reasonable measure of separability. Furthermore, it shows us that the optimal threshold for the LDA model is at 51.9%. The optimal LDA model has a specificity of 79.5%, which is higher than that of the optimal logistic model, despite similar AUCs. Therefore, we are inclined to recommend LDA over logistic regression.  
...
We then checked to see if QDA could outperform LDA -- which might be the case if covariation between input variables were different across classes. Results are below.  
```{r}
...
##TESTING DIFFERENT THRESHOLDS (DELETE IF UNUSED)
#qda.cv.tab <- table(predicted = (qda.cv.pred$posterior[,"Yes"] > 0.218),
#                    actual = val.dat$two_year_recid)
```
...
```{r}
kable(recid.rf$confusion)
rf.error = (recid.rf$confusion[2,1] + recid.rf$confusion[1,2]) /nrow(train.dat)
```

Surprisingly, this matrix shows that the overall accuracy rate for this random forest is ``r scales::percent(1 - rf.error, accuracy = 0.1)``, which is not substantially better than our other models.
...
recid.vio.lda.pred.train <- predict(recid.vio.lda,
                          type = "response")
#table(predicted = (qda.cv.pred$posterior[,"Yes"] > 0.218),
#                    actual = val.dat$two_year_recid)
rate.vio2 <- mean(train.dat$vio_recid_2yr == "Yes")
...
The output shows a classification tree that predicts that nobody will violently recidivate. It is a bit odd that the tree still splits on priors, but the regression tree sheds some light on that. Matching the classification tree, both branches lead to "no", but the ROC curve shows us that this is because the model used an inappropriate 50% cutoff.  
...
This is clearly not a particularly useful model. Even with the best threshold, sensitivity and specificity are quite low.
...
The confusion matrix for our random forest created using training data is below:
```{r}
kable(recid.vio.rf$confusion[,1:2])
...
| <span style ="color: blue;"> Logistic regression |<span style ="color: blue;">`r scales::percent(logit.acc, 0.1)`</span> |<span style ="color: blue;"> `r scales::percent(logit.sens, 0.1)`</span>   |<span style ="color: blue;">`r scales::percent(logit.spec)`</span> |  
| Linear Discriminant Analysis (LDA)         |`r scales::percent(lda.cv.acc, 0.1)`  | `r scales::percent(lda.cv.sens, 0.1)`  |`r scales::percent(lda.cv.spec, 0.1)`|  
| Quadratic Discriminant Analysis (QDA)      |`r scales::percent(qda.cv.acc, 0.1)`  | `r scales::percent(qda.cv.sens, 0.1)`  |`r scales::percent(qda.cv.spec, 0.1)`|  
| Decision Tree                              |`r round(tree.vals$accuracy, 1)`% | `r round(tree.vals$sensitivity, 1)`% | `r round(tree.vals$specificity, 1)`% |  
| Random Forest (RF)                         |`r round(rf.vals$accuracy, 1)`% | `r round(rf.vals$sensitivity, 1)`% | `r round(rf.vals$specificity, 1)`% |  
...
|<span style ="color: blue;"> Logistic regression</span>|<span style ="color: blue;">`r scales::percent(logit.acc.vio, 0.1)`</span> |<span style ="color: blue;">`r scales::percent(logit.sens.vio, 0.1)`</span> |<span style ="color: blue;">`r scales::percent(logit.spec.vio, 0.1)`</span>|  
| Linear Discriminant Analysis (LDA)         |`r scales::percent(lda.cv.acc.vio, 0.1)` |`r scales::percent(lda.cv.sens.vio, 0.1)` |`r scales::percent(lda.cv.spec.vio, 0.1)`|  
| Quadratic Discriminant Analysis (QDA)      |`r scales::percent(qda.cv.acc.vio, 0.1)` |`r scales::percent(qda.cv.sens.vio, 0.1)` |`r scales::percent(qda.cv.spec.vio, 0.1)`|  
| Decision Tree                              |`r round(tree.vals.vio$accuracy, 1)`% |`r round(tree.vals.vio$sensitivity, 1)`% |`r round(tree.vals.vio$specificity, 1)`% |  
| Random Forest (RF)                         |`r round(rf.vals.vio$accuracy, 1)`% | `r round(rf.vals.vio$sensitivity, 1)`% | `r round(rf.vals.vio$specificity, 1)`% |  
...
Our final model for violent recidivism is also a logistic regression, because again it was (together with LDA) a top-performing model. We chose the optimal threshold of 0.103 because it offered strong sensitivity as well as specificity, both of which are important for predicting violent recidivism.
...
<span style ="color: red;"> Add text here about over-fitting </span>
...
From the Gender table below we see that our model strongly favors giving female individuals a lower recidivism risk rating. The False Negative rate of `r scales::percent(FN.any.female, 0.1)` is especially telling, since this implies that for every hundred recidivating women, our model says roughly 86 of them are not likely to recidivate. This rate is double the False Negative rate for males. 
...
This 100% false negative rate for ages 45 and above confused us originally, but we would this was a product of the small sample size with the test dataset. Even though re-running the analysis with the full dataset did not give the same "fresh" perspective, the table below shows what the numbers look like with a larger sample size.  
...
#       format = "markdown", caption = "**Success of the COMPAS instrument**")
...
It is more than an unfortunate coincidence that both the COMPAS model and our model have a strong racial bias. Crime data in the United States, including recidivism data, is highly biased because police are more likely to arrest African Americans than white people for the same crimes. Any model based on American crime data, no matter how internally sound, will reflect the biased data from which it was built.

<span style ="color: red;"> If somewhere someone can write what the most important predictors were for vio and how they compare to any recidivism!! I know this is somewhere but it's a prompt question and I forget where and my brain is now tired.<span>

#### **3a.3: Are there systematic differences between our classifications and those of COMPAS?**  

As stated earlier, there are similarities between our classifications and those of COMPAS in regards to the biases of how our model classifies individuals. However, since we also favored Specificity in our model for Any Recidivism data, our logistic model for Any Recidivism does optimize towards having a higher Specificity rate. This is shown through our models having a lower False Positive rate on average than the COMPAS model as shown in sections 2c and 2d <span style ="color: red;">(If anyone has time/feels the need, we could link to the sections somehow...not sure how to do that)</span>. 

For Violent Recidivism data, our model systematically optimizes towards results that produce higher Sensitivity rates than the COMPAS model as well. This is shown in sections 2c and 2d as well in our model resulting in lower False Negative rates (and as a result higher False Positive rates) for the violent recidivism data as compared to the model for the any recidivism data.
<span style ="color: red;"> This is not true for the Age analysis so someone please check me on this!!<span>

<span style ="color: red;"> I just remembered, but forgot before, did we want to mention the not-including/we ended up including race_factor in our model and how this doesn't really change much? I know we mentioned it somewhere in part 2, but maybe it could go in 3a.3?<span>

### **3b: Reflections on risk assessment tools**   
**update needed: create this whole section after finishing EVERYTHING ELSE**  

The problem of prediciting recidivism rates is inherently random. We know that it is random because it is very possible to find two individuals with the exact same statistics who end up with opposite outcomes (one will recidivate within two years, and the other will not). This means that the data is extremely challenging to make predictions with using today's Data Mining tools, especially provided the available data, which itself contains racist/sexist/ageist biases. 

<span style ="color: red;"> This is a little biased/very opinionated, so idk if this should be left here:<span>
We show in this report that with fewer variables, data mining tools available to the average student and a limited amount of time, we were able to replicate and adjust a model that was  manufactured for government use. While it is important for models to be tested, is it also important that until they are proven ready to be fair to the people they are affecting, they should not play a role in any decision making. We do believe that there is value to this research, but the research should go through extensive trials before it is rolled out for government/private use. <span style ="color: red;"> It's possible I'm misremembering what they wrote in ProPublica, but I thought several states were using combined assessments with these models and judges.<span>

<span style ="color: red;"> Obviously this needs to be completed :) I tried to explain inherent randomness of the problem, but if someone has a better explanation, please provide! Scrap whatever you think is best to scrap. I will not be offended. -- I think of course it is possible for someone to make the statement that recidivism can be predicted, but I really do think that we often find people who have the same stats but who have different outcomes, and I think explaining it is random is further proof that this data should not be predicted with today's machine learning, ESPECIALLY since the data it's learning on is inherently biased! You never know, future methods might be able to account for biases. This is obviously not to say that people who work on this should stop working on this. I think it is still a problem worth working on, but it should not, at its current capacity, be used to make decisions that affect people's lives. Also since current model is based on biases, then this could re-enforce judges' biases when they see the prediction model agree with them...idk. THOUGHTS.<span>
...
<span style ="color: red;"> Fix me, please!! Do we want to be showing the code here?<span>

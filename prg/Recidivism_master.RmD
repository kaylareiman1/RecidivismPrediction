...
Given our task of predicting binary outcomes, this is fundamentally a classification problem. Informed by [our book - ISLR](https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=1&cad=rja&uact=8&ved=2ahUKEwjx8tmdyafpAhVxg3IEHWqjDIsQFjAAegQIAhAB&url=https%3A%2F%2Ffaculty.marshall.usc.edu%2Fgareth-james%2FISL%2FISLR%2520Seventh%2520Printing.pdf&usg=AOvVaw3IIbJOiIiKLgG0eFhvQBp9) <span style = "color: red;">(do we need to cite the textbook here? -M)</span>, we decided to pursue the following methods:  
   ...
   3) **Quadratic Discriminant Analysis (QDA)**: This is like LDA but more flexible. It works when there are class-based interactions among the predictors, so we will try this out in addition to LDA in case this is the case.   
   4)  **Classification and Regression Trees**: Using a decision tree to classify our respondents is helpful if the relationship between variables is non-linear. Additionally, classification trees can be interpreted graphically, a major advantage in a model that requires public transparency. Trees can approximate human decision-making processes, which makes their interpretation more intuitive to the general public.  However, the disadvantage is that trees are unstable. That is, a few new observations can completely change which variable is being used for the first split. We also examine regression trees which treat our outcome binary variable as numeric. 
...
```{r}
#Validation Time!!
lda.pred.val <- predict(object = recid.lda, val.dat, type = "response")
...
```{r}
#Run original tree
tree.recid <- tree(formula = fullmodel, data = train.dat)
#Create summary object
summary.tree.recid <- summary(tree.recid)
```
...
#Again, predict and use the validation data
tree.pred.prune=predict(prune.recid, val.dat, type="class")
table(tree.pred.prune,val.dat$any_recid_2yr)
tree.pred.prune.errors = table(tree.pred.prune,val.dat$any_recid_2yr)[2,1] + table(tree.pred.prune,val.dat$any_recid_2yr)[1,2]
...
**Regression trees**
Now we tried regression trees. Even though our data are not continuous, our factor variable is binary and therefore can be approached numerically. And as we learned in class, a model can fail to meet certain assumptions while still providing a high level of prediction value in classification Therefore, we decided to experiment. This work is shown below. The regression tree also allowed us to look at tradeoffs in sensitivity and specificity via an ROC curve - which were not possible with a classification tree.  

```{r}
#Create model using rpart functiona and training dataset
tree.recid.rpart.train <- rpart(formula = fullmodel, data = train.dat)
#Output pretty picture
tree.recid.rpart.train.pretty <- as.party(tree.recid.rpart.train)
plot(tree.recid.rpart.train.pretty, pretty = 1)
print(tree.recid.rpart.train.pretty)
...
#Create ROC curve
roccurve.rpart<- roc(response = val.dat$two_year_recid, predictor = tree.pred.rpart.val[,2], levels = c(0, 1), direction = "<")

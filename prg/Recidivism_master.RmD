...
```{r}
##Violent
final.predict.test.vio = predict(glm.logit, test.dat, type="response") 
final.predict.test.vio = ifelse(final.predict.test.vio > 0.6, "1", "0") #threshold of 0.6
...
After choosing the best version of each model, we assessed the potential for prejudice. Our goal was to replicate Pro-Publica's chart for each of these metrics included in the introduction. We will use our test dataset for this exercise, since it wasn't used to build the models. The table below shows the number of African-American and white people in our test data.
...
|--------------------------------------------|-----------------|------------------|
| Labeled Higher Risk, But Didn’t Re-Offend  (False positive)|          |         |
| Labeled Lower Risk, Yet Did Re-Offend      (False Negative)|          |         | 
